...
case "${PRIVATE_VAULT_BUILD_JOB_CODEC_TESTS:-}" in
  "") ;;
  1)
  JOB_CODEC_TEST_OUTPUT="$OUTPUT_ROOT/.job-codec-tests"
  rm -rf "$JOB_CODEC_TEST_OUTPUT"
  mkdir -p "$JOB_CODEC_TEST_OUTPUT"
  compile_job_codec_test_slice() {
    local architecture="$1"
    local sodium_root="$2"
    local output="$JOB_CODEC_TEST_OUTPUT/private-vault-job-codec-tests-$architecture"
    xcrun clang -O1 -fobjc-arc -fblocks -Wall -Wextra -Werror \
      -isysroot "$SDK" -mmacosx-version-min=13.0 -arch "$architecture" \
      ...
      "$SOURCE_ROOT/control/PrivateVaultAncCanonical.m" \
      "$SOURCE_ROOT/control/PrivateVaultJobCodec.m" \
      "$SOURCE_ROOT/control/PrivateVaultJobCodecTests.m" \
      "$sodium_root/lib/libsodium.a" -o "$output"
    lipo "$output" -verify_arch "$architecture"
  }
  compile_job_codec_test_slice arm64 "$ARM64_SODIUM"
...
case "${PRIVATE_VAULT_BUILD_ENROLLMENT_CHALLENGE_TESTS:-}" in
1 | true | TRUE | yes | YES)
  ENROLLMENT_CHALLENGE_TEST_OUTPUT="$OUTPUT_ROOT/.enrollment-challenge-tests"
  rm -rf "$ENROLLMENT_CHALLENGE_TEST_OUTPUT"
  mkdir -p "$ENROLLMENT_CHALLENGE_TEST_OUTPUT"
  build_enrollment_challenge_tests() {
    local architecture="$1"
    local sodium_root
    if [[ "$architecture" == "arm64" ]]; then sodium_root="$ARM64_SODIUM"; else sodium_root="$X86_64_SODIUM"; fi
    local output="$ENROLLMENT_CHALLENGE_TEST_OUTPUT/private-vault-enrollment-challenge-tests-$architecture"
    xcrun clang -O1 -fobjc-arc -fblocks -Wall -Wextra -Werror \
      -isysroot "$SDK" -arch "$architecture" -mmacosx-version-min=13.0 \
      -I"$SOURCE_ROOT/crypto" -I"$SOURCE_ROOT/control" \
      -I"$sodium_root/include" -framework Foundation \
      ...
      "$SOURCE_ROOT/control/PrivateVaultEnrollmentOffer.m" \
      "$SOURCE_ROOT/control/PrivateVaultEnrollmentChallenge.m" \
      "$SOURCE_ROOT/control/PrivateVaultEnrollmentChallengeTests.m" \
      "$sodium_root/lib/libsodium.a" -o "$output"
    lipo "$output" -verify_arch "$architecture"
  }
  build_enrollment_challenge_tests arm64
  if [[ "$PRIVATE_VAULT_BUILD_ARCHITECTURES" == "universal" ]]; then
    build_enrollment_challenge_tests x86_64
  fi
  ;;
esac

case "${PRIVATE_VAULT_BUILD_EEK_WRAP_TESTS:-}" in
1 | true | TRUE | yes | YES)
  EEK_WRAP_TEST_OUTPUT="$OUTPUT_ROOT/.eek-wrap-tests"
  rm -rf "$EEK_WRAP_TEST_OUTPUT"
  mkdir -p "$EEK_WRAP_TEST_OUTPUT"
  build_eek_wrap_tests() {
    local architecture="$1"
    local sodium_root
    if [[ "$architecture" == "arm64" ]]; then sodium_root="$ARM64_SODIUM"; else sodium_root="$X86_64_SODIUM"; fi
    local output="$EEK_WRAP_TEST_OUTPUT/private-vault-eek-wrap-tests-$architecture"
    xcrun clang -O1 -fobjc-arc -fblocks -Wall -Wextra -Werror \
      -isysroot "$SDK" -arch "$architecture" -mmacosx-version-min=13.0 \
      -I"$SOURCE_ROOT/crypto" -I"$SOURCE_ROOT/control" \
      -I"$sodium_root/include" -framework Foundation \
      ...
      "$SOURCE_ROOT/control/PrivateVaultAncCanonical.m" \
      "$SOURCE_ROOT/control/PrivateVaultEekWrap.m" \
      "$SOURCE_ROOT/control/PrivateVaultEekWrapTests.m" \
      "$sodium_root/lib/libsodium.a" -o "$output"
    lipo "$output" -verify_arch "$architecture"
  }
...
case "${PRIVATE_VAULT_BUILD_BROKER_REPLACEMENT_APPROVAL_TESTS:-}" in
  "") ;;
  1 | true | TRUE | yes | YES)
    BROKER_REPLACEMENT_APPROVAL_TEST_OUTPUT="$OUTPUT_ROOT/.broker-replacement-approval-tests"
    rm -rf "$BROKER_REPLACEMENT_APPROVAL_TEST_OUTPUT"
    mkdir -p "$BROKER_REPLACEMENT_APPROVAL_TEST_OUTPUT"
    build_broker_replacement_approval_tests() {
      local architecture="$1"
      local sodium_root
      if [[ "$architecture" == "arm64" ]]; then sodium_root="$ARM64_SODIUM"; else sodium_root="$X86_64_SODIUM"; fi
      local output="$BROKER_REPLACEMENT_APPROVAL_TEST_OUTPUT/private-vault-broker-replacement-approval-tests-$architecture"
      xcrun clang -O1 -fobjc-arc -fblocks -Wall -Wextra -Werror \
        -isysroot "$SDK" -arch "$architecture" -mmacosx-version-min=13.0 \
        -I"$SOURCE_ROOT/crypto" -I"$SOURCE_ROOT/control" \
        -I"$SOURCE_ROOT/storage" -I"$SOURCE_ROOT/recovery" \
      ...
      "$SOURCE_ROOT/storage/PrivateVaultCustodyRepository.m" \
        ...
        "$SOURCE_ROOT/transport/PrivateVaultBootstrapFrame.m" \
        "$SOURCE_ROOT/transport/PrivateVaultBootstrapReplay.m" \
        "$SOURCE_ROOT/control/PrivateVaultBrokerReplacementApprovalTests.m" \
        "$sodium_root/lib/libsodium.a" -o "$output"
      lipo "$output" -verify_arch "$architecture"
    }
    build_broker_replacement_approval_tests arm64
    if [[ "$PRIVATE_VAULT_BUILD_ARCHITECTURES" == "universal" ]]; then
      build_broker_replacement_approval_tests x86_64
    fi
    ;;
  *) echo "Invalid Private Vault broker-replacement-approval-test build mode" >&2; exit 1 ;;
esac
